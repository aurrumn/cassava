...
class c_bos_petani extends CI_Controller {

    function __construct() {
        parent::__construct();
        $this->load->library('template');
        $this->load->model('model_bos/m_bos_petani');
    ...
    function index() {
        if ($this->cekLogin() == FALSE) {
            ...
            $data['data_petani'] = $this->m_bos_petani->ambilDataPetani();
            $this->template->bosview('bos/bos_petani', $data);
        }
    }
    ...
    function non_AktifkanPetani($id) {
        ...
        if ($this->m_bos_petani->update_status_petani("petani", $data, "id_petani = '$id'")) {
            redirect('controller_bos/c_bos_petani');
        } else {
    ...
    function edit_data_petani() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($this->m_bos_petani->update_data_petani("petani", $data, "id_petani = '$id_petani'")) {
                redirect('controller_bos/c_bos_petani');
            } else {
                $this->session->set_flashdata('message', 'Proses Gagal');
                redirect('controller_bos/c_bos_petani');
    ...
    function ajax_get_data_petani($id) {
        $data = $this->m_bos_petani->get_data_petani_for_edit("petani", $id);
